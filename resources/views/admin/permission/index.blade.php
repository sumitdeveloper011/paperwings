    ...
    <div class="modern-card modern-card--compact">
        ...
        <div class="modern-card__body">
            <div id="permissions-table-container">
                @include('admin.permission.partials.table', ['permissions' => $permissions])
            </div>

            <div id="permissions-pagination-container" class="pagination-wrapper">
                @if($permissions->hasPages())
                    {{ $permissions->appends(request()->query())->links('components.pagination') }}
                @endif
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof AdminSearch !== 'undefined') {
        AdminSearch.init({
            ...
            resultsContainer: '#permissions-table-container',
            paginationContainer: '#permissions-pagination-container',
            loadingIndicator: '#search-loading',
            searchUrl: '{{ route('admin.permissions.index') }}',
            debounceDelay: 300
        });
    }
});
